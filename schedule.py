...
class AEModel:
    ...
    def attend_ae(self, patient):
        ...
        if self.env.now > p.warm_up:
            ...
            track.results_df = (track.results_df.copy() if df_to_add.empty else df_to_add.copy() if track.results_df.empty else pd.concat([track.results_df, df_to_add]) # if both DataFrames non empty
      )
